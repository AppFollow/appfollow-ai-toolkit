...
| `get_server_info` | deployed build sha + liveness |
...
| Tool | Purpose |
|---|---|
| `get_reviews` | paged raw reviews for ONE app — `ext_id` required; for a collection loop `list_apps`'s ext_ids; page until `reviews.page.next` is null (evidence) |
